...
${e}`);return}(await navigator.serviceWorker.ready).showNotification(a,{body:e,icon:"/images/android-chrome-192x192.png",badge:"/images/android-chrome-192x192.png"})}function q(a){const e="=".repeat((4-a.length%4)%4),t=(a+e).replace(/-/g,"+").replace(/_/g,"/"),i=atob(t);return new Uint8Array([...i].map(n=>n.charCodeAt(0)))}const H={BASE_URL:"https://story-api.dicoding.dev/v1"},E=H.BASE_URL;async function x(){const a=localStorage.getItem("token");return(await(await fetch(`${E}/stories`,{headers:{Authorization:`Bearer ${a}`}})).json()).listStory||[]}async function j({description:a,photo:e,lat:t,lon:i}){const n=localStorage.getItem("token"),o=new FormData;return o.append("description",a),o.append("photo",e),t&&i&&(o.append("lat",t),o.append("lon",i)),await(await fetch(`${E}/stories`,{method:"POST",headers:{Authorization:`Bearer ${n}`},body:o})).json()}async function z({email:a,password:e}){return await(await fetch(`${E}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:a,password:e})})).json()}async function _({name:a,email:e,password:t}){return await(await fetch(`${E}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:a,email:e,password:t})})).json()}function G(a){typeof(a==null?void 0:a.onLoading)=="function"&&a.onLoading(),x().then(e=>{typeof(a==null?void 0:a.onSuccess)=="function"&&a.onSuccess(e)}).catch(e=>{console.error("Gagal mengambil data cerita:",e),typeof(a==null?void 0:a.onError)=="function"&&a.onError(e)})}function F({description:a,photo:e,lat:t,lon:i}){return j({description:a,photo:e,lat:t,lon:i})}class K{async render(){return`
    ...
    `:(setTimeout(()=>{window.location.hash="#/login"},100),'<section class="container"><p>Anda harus login untuk menambahkan cerita.</p></section>')}async afterRender(){const e=document.getElementById("preview-container"),t=n=>{const o=new FileReader;o.onload=r=>{e.innerHTML=`<img src="${r.target.result}" alt="Preview" style="max-width:100%; border-radius:6px;" />`},o.readAsDataURL(n)};document.getElementById("open-folder").addEventListener("click",()=>{document.getElementById("file-input").click()}),document.getElementById("file-input").addEventListener("change",n=>{d=n.target.files[0],t(d)});const i=/Android|iPhone|iPad|iPod/i.test(navigator.userAgent);document.getElementById("open-camera").addEventListener("click",async()=>{if(i)document.getElementById("mobile-camera-input").click();else{const n=document.getElementById("webcam-container"),o=document.getElementById("webcam"),r=document.getElementById("snapshot"),m=document.getElementById("capture-photo");n.style.display="block";try{g=await navigator.mediaDevices.getUserMedia({video:!0}),o.srcObject=g}catch(T){alert("Tidak dapat mengakses webcam: "+T.message)}m.onclick=()=>{const T=r.getContext("2d");r.width=o.videoWidth,r.height=o.videoHeight,T.drawImage(o,0,0),r.toBlob(P=>{d=new File([P],"webcam-capture.jpg",{type:"image/jpeg"}),t(d)},"image/jpeg"),g&&(g.getTracks().forEach(P=>P.stop()),o.srcObject=null,g=null),n.style.display="none"}}}),document.getElementById("mobile-camera-input").addEventListener("change",n=>{d=n.target.files[0],t(d)}),W("map",(n,o)=>{p(this,k,n),p(this,w,o)}),window.addEventListener("hashchange",()=>{g&&(g.getTracks().forEach(n=>n.stop()),g=null)}),document.getElementById("story-form").addEventListener("submit",async n=>{n.preventDefault();const o=document.getElementById("form-message");o.textContent="Mengirim...";const r=document.getElementById("description").value;if(!d){o.textContent="Silakan pilih atau ambil foto terlebih dahulu.";return}const m=await F({description:r,photo:d,lat:s(this,k),lon:s(this,w)});m.error?o.textContent=`Gagal: ${m.message}`:(o.textContent="Cerita berhasil dikirim!",n.target.reset(),e.innerHTML="",d=null,this.marker&&map.removeLayer(this.marker),setTimeout(()=>{location.hash="#/"},1500))})}}k=new WeakMap,w=new WeakMap;async function Y(a,e,t,i){try{const n=await z({email:a,password:e});t(n)}catch(n){console.error("Login gagal:",n),i({error:!0,message:"Login gagal. Silakan coba lagi."})}}async function Z(a,e,t,i,n){try{const o=await _({name:a,email:e,password:t});i(o)}catch(o){console.error("Registrasi gagal:",o),n({error:!0,message:"Registrasi gagal. Silakan coba lagi."})}}class Q{async render(){return`
    ...
    `}async afterRender(){const e=document.querySelector("#register-form"),t=document.querySelector("#register-message");e.addEventListener("submit",i=>{i.preventDefault();const n=e.name.value,o=e.email.value,r=e.password.value;Z(n,o,r,m=>{t.textContent=m.message,e.reset(),setTimeout(()=>{location.hash="#/login"},1500)},m=>{t.textContent=m.message})})}}function D(a){const e=a.split("/");return{resource:e[1]||null,id:e[2]||null}}function ee(a){let e="";return a.resource&&(e=e.concat(`/${a.resource}`)),a.id&&(e=e.concat("/:id")),e||"/"}function C(){return location.hash.replace("#","")||"/"}function te(){const a=C(),e=D(a);return ee(e)}function ae(){const a=C();return D(a)}var h;class ne{constructor(e){l(this,h);p(this,h,e)}async showDetail(e){try{const i=(await x()).find(n=>n.id===e);i?s(this,h).displayStory(i):s(this,h).displayError("Cerita tidak ditemukan.")}catch{s(this,h).displayError("Gagal mengambil data cerita.")}}}h=new WeakMap;var S;class ie{constructor(){l(this,S)}async render(){return`
      <section class="container">
